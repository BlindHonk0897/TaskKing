...
var gap = [];
if(studA.length > studB.length){gap.push(studA.length - studB.length);gap.push("B")}
else{gap.push( studB.length - studA.length);gap.push("A")}

console.log(gap);

newKitch.push(kitchC);
newdish.push(dishC);
newdin.push(dinC);
...
if(gap[1]==="A"){
    if(gap[0]===1){
      var ln = getRandomFromB();
      newOff.push(ln);
      studB = studB.filter(function(item) {
         return item.StudentLastname !== ln; 
      });
    }else{
      var ln = getRandomFromB();
   ...
   if(gap[0]===1){
      var ln = getRandomFromA();
      newOff.push(ln);
      studA = studA.filter(function(item) {
         return item.StudentLastname !== ln; 
      });
   }else{
      var ln = getRandomFromA();
...
if((studA.length - studB.length)===1){
   var ln = getRandomFromA();
      newcomp.push(ln);
      studA = studA.filter(function(item) {
         return item.StudentLastname !== ln; 
   });
}else if((studA.length - studB.length)===2){
   var ln = getRandomFromA();
   newcomp.push(ln);
   studA = studA.filter(function(item) {
      return item.StudentLastname !== ln; 
...
var Coordinator = getRandomCoordinator(newBG);
